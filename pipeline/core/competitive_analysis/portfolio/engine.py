"""PortfolioReportEngine — wires research/compiled data into a long-form LLM call.
...
report in a single call. If no research/compiled data is found for the requested
...
from pipeline.core.competitive_analysis.portfolio.prompts import get_theme_prompt
from pipeline.core.competitive_analysis.portfolio.theme import PortfolioReportRequest
from pipeline.core.competitive_analysis.wiki_query import WikiQuery
from pipeline.core.llm_wiki.ingest import LLMClient
...
class PortfolioReportEngine:
    ...
    async def generate(self, request: PortfolioReportRequest) -> str:
        ...
        if not product_facts:
            logger.warning(
                "No research/compiled data for %s on dims %s",
                request.product_ids,
                dims,
            )
