...
def ingest(
    ...
    _not_implemented(
        ...
        "pipeline.core.llm_wiki.sync.sync_product_path_a"
    )


@app.command(help="Compile raw → compiled (full sync, batch)")
def compile(
    products_file: Path = typer.Option(Path("pipeline/products/coding-agents.yaml")),
    ...
    typer.echo("请改用 `wiki path-b`,或直接调用 pipeline.core.llm_wiki.sync.sync_product_path_a")
    raise typer.Exit(1)
...
def compare(
    ...
    products_file: Path = typer.Option(
        Path("pipeline/products/coding-agents.yaml"), "--products-file"
    ),
    dims_file: Path = typer.Option(
        Path("research/schema/coding-agent-dims.yaml"), "--dims-file"
    ),
...
def path_b(
    ...
    dims_path = Path("research/schema/coding-agent-dims.yaml")
...
def portfolio(
    ...
    output_dir: Path = typer.Option(
        Path("research/reports/portfolio"), "--output-dir"
    ),
