# E1｜什么是 Harness：为什么用了最强模型，产品还是不如对手

> 草稿版本：v2 · 状态：✍️ 草稿中 · 字数目标：6000-8000
>
> v2 相对 v1 的变化：按"什么是→为什么→有哪些→怎么判断→谁做得好→对 PM 的要求"6 问重排；新增七层打分表；9 款产品用上新补的 Copilot / OpenCode / Cowork 档案；能力模型收敛为"架构设计 + 评估"双核闭环。
...
|---|---|---|
| **E2 循环与编排** | 贯穿 E/T/L 的"主循环" | Agent 怎么 Think-Act-Observe？怎么不死循环跑飞？ |
| **E3 工具系统与 MCP** | **T 工具接口** | 怎么让 Agent 调对工具、调安全？ |
| **E4 上下文与记忆** | **C 上下文** | 一次对话带着什么？怎么组装 / 压缩 / 缓存，记忆怎么不失控？ |
| **E5 多 Agent** | 多 Agent（Skills / 子 Agent / 多 Agent） | 能力多了、任务大了怎么拆、怎么管？ |
...
| 产品 | 一句话哲学 | 它最强在哪（说人话） | 档案 |
|---|---|---|---|
| **Claude Code** | "笨循环"——把秩序放进 Harness，而不是堆在模型里 | 把"该怎么干活"的规矩都内建好了，长任务、复杂活儿跑得最稳 | `entities/claude-code.md` |
| **Codex** | OpenAI 的极简主义运行框架，模型+工具+状态的循环 | 全程看得见它在想什么、做什么，还能随时打断插话 | `entities/codex.md` |
| **Cursor** | 从编辑器一路进化到 Agent Mode | 只读跟当前任务相关的内容，省钱、不跑偏 | `entities/cursor.md` |
| **Trae** | SOLO 多 Agent 覆盖从需求到部署的全流程 | 一条龙：从需求到上线，全用多个 Agent 接力干完 | （本章一句话带过） |
| **GitHub Copilot** | 不另起炉灶，把 Agent 接进 GitHub 已有的秩序 | 治理安全 G（防火墙 / 审计 / 防注入） | `entities/github-copilot.md` |
| **OpenCode** | 开源阵营：客户端-服务端分离 + 可治理的开放 Harness | 开源也能管得细：谁能用、能调什么，都拦得住 | `entities/opencode.md` |
| **Claude Cowork** | 把 Claude Code 的执行力搬到知识工作 | 先给计划等你点头再动手，还能定时自动干活 | `entities/claude-cowork.md` |
| **Hermes Agent** | 自进化：把轨迹 / Skill / RL 训练纳入闭环 | 自进化（见 E7） | `entities/hermes-agent.md` |
| **Qoder / CodeBuddy** | 国产 Harness Engineering：专家团 + 终端沙箱 | 像一支专家小队，在隔离的安全环境里干活 | `entities/qoder.md`、`entities/codebuddy.md` |
| **Manus** | 上下文工程做得好的通用任务 Agent | 特别会"喂材料"，给它的信息总是又准又够用 | （本章先不展开） |

### 5.1 我高强度用下来，四款各一句真实体感

光看定位没用，下面这几款是我自己长期用过的，各讲一句真实感受——也顺手标出它各自在 ETCLOVG 哪一层立住了：

- **Claude Code**：最大的感受不是它代码写得多好，而是它**定义了一整套行业标准**——`CLAUDE.md`（项目规则注入）、MCP（工具协议）、Skills（能力封装）现在几乎成了别家都在抄的事实标准；而且**长任务、复杂任务**它明显更稳。换句话说，它强在把执行秩序（C/T + 多 Agent）做成了行业范式。
...
- **Trae**：它的 **SOLO 模式用多 Agent 覆盖了从需求分析、任务拆解、编码、测试到部署的完整闭环**,AI 主导执行、开发者只管验收；而且**规则和记忆能分全局级和项目级设置**，还能把指定文档塞进上下文。这是"多 Agent + 上下文"两层一起发力。
...
E2《循环与编排：任务怎么"跑起来"又不跑飞》——从"Agent 卡住的几种常见死法"讲起，看腾讯那 4 亿 token 是怎么烧掉的，以及几道硬闸该怎么设计。

---

<!-- 配图清单（待重绘为定稿质量）：
  1. 五阶段范式演进（本文已有 Mermaid）
  2. Harness as OS 主干图（本文已有 Mermaid；完整版见 assets/harness-as-os-overview.jpg,E4 详拆）
  3. ETCLOVG 七层打分表（已是 Markdown 表）
  4. 10 款定位速览表（已是 Markdown 表）
  5. 双核能力闭环图（本文已有 Mermaid）
-->
<!-- 素材状态：
  - ✅ 已补档案：entities/github-copilot.md、entities/opencode.md、entities/claude-cowork.md
  - ✅ 一手观察已填：2.5 手电筒反例 + 5.1 四款真实体感（开篇觉醒场景与 DeepSeek 留白按周浩要求移除）
  - ⚠️ Manus 仅一句话带过（上下文工程），无独立档案
-->
