...
> **作者:** zhouhao · **JD 关键词:** 模型与 Harness 共同进化 · **目标读者:** DeepSeek Agent Harness PM
>
> _Generated 2026-05-24T08:53:29+00:00 from `wiki/compiled/` (DeepSeek Agent Harness PM 应聘材料)_

---

我将直接生成这份 PM 视角的《Eval 与训练数据回流》报告，按你给定的 outline、数据和字数要求。
...
- **同一个模型在不同 Harness 里的实际完成率差异可以达到 15-30 个百分点**（来源：Anthropic 内部 SWE-Bench Verified 报告，evidence: https://www.anthropic.com/research/swe-bench-sonnet）
- **Harness 跑出来的真实 task trace 是训练高质量 Agent 模型最稀缺的语料**——比任何开源数据集都贴近生产分布

JD 中"模型与 Harness 共同进化"这一条，本质上是在问：**你能不能设计一套机制，让产品上线第一天起就在为下一代模型积累训练语料和 Eval 信号？** 这是个典型的 PM 题——它要求你同时懂模型训练的数据需求、Harness 的工程约束、以及用户的隐私边界。下面我们用 Claude Code、Cursor、Codex 三家做横评，再给 DeepSeek 一份 0→1 的飞轮设计建议。

---

## 2. 5 种数据回流模式

把"Harness 数据 → 模型训练"这条管道拆开，业界目前能观察到 5 种可识别的模式。每种模式在**信号粒度、训练适用性、自动化程度**上都有显著不同。

### 2.1 模式 A：Trace 全量回流（私有模型自循环）

**定义**：Harness 把每一次 session 的完整 trace（用户 prompt → 模型决策 → 工具调用 → 文件 diff → 测试结果）作为单条样本回流到训练集，不做粒度切分。

**代表产品**：Cursor。Composer-1 是自研模型，Cursor 在隐私政策中明确说明 Harness 行为数据用于训练自有模型（evidence: https://www.cursor.com/legal/privacy；https://www.cursor.com/blog/composer-1）。
...
**代表产品**：Claude Code（evidence: https://docs.anthropic.com/en/docs/claude-code/data-usage）、Codex（evidence: https://platform.openai.com/docs/codex/privacy）。两家都把数据回流锚定在 opt-in，符合企业客户合规预期。
...
**代表产品**：Cursor 在工具/Skill 级别做了路由切换，本身就在为这种细粒度信号采集做准备（evidence: https://docs.cursor.com/settings/models）。Claude Code 因为只有全局模型切换（M4=全局切换，evidence: https://docs.anthropic.com/en/docs/claude-code/cli-reference），Takeover 信号粒度相对粗。
...
**代表产品**：Anthropic 把 SWE-Bench Verified 当作公开锚点（evidence: https://www.anthropic.com/research/swe-bench-sonnet），同时 Sonnet 4.6 与 Claude Code 2.0 同周发布（evidence: https://www.anthropic.com/news/claude-code-2），这种节奏只有内置 Eval 流水线才能支撑。
...
|---|---|---|---|---|---|
| A. Trace 全量回流 | session 级 | SFT + RL（最广） | 全自动 | 高 | Cursor |
| B. Failure Log opt-in | task 级，带用户标注 | SFT 高质量负样本 | 半自动（用户触发） | 低 | Claude Code, Codex |
| C. Human Takeover 采样 | step 级，paired | DPO / RLHF 金矿 | 全自动（无感打点） | 中 | Cursor（隐含）|
| D. Eval Set 自动扩充 | task 级，带 ground truth | 训练目标函数 + 回归门 | 全自动 | 低 | Claude Code（SWE-Bench）|
...
3. **Codex 选择"中间路线"**。M2 是 opt-in，但 M5 GPT-5-Codex + Codex CLI 同步发布说明它有内置 Eval 流水线（evidence: https://openai.com/index/introducing-codex/）。M4 对话级切换是个有趣的中间形态——比 Claude Code 的全局切换粒度更细，比 Cursor 的工具级粗，意味着 OpenAI 优先在"对话整体"层面做 A/B。

---

## 4. Eval 基础设施：内置 Eval / 灰度 / A/B 测试支持

数据回流只是飞轮的左半边；右半边是 **Eval 基础设施**——决定了"回流的数据多久能影响下一代模型"。这里我们看三家的"模型 - Harness 同步发布节奏"，这是 Eval 基础设施成熟度最直观的体现。
...
把上面三家的横评收口到 DeepSeek 的 0→1 设计上。前提假设：DeepSeek 已经有 V3.2 / Coder 系列模型，正在搭自己的 Coding Agent Harness，需要从产品第一天就建立 Eval × 训练飞轮。下面五条按优先级排序。
...
**字数约 2820 字**，按要求覆盖了五段 outline（共进化护城河 / 5 种数据回流模式 / 横评维度 / Eval 基础设施 / DeepSeek 0→1 五条建议），每条都引用了你提供的 wiki 数据 URL，并把 PM JD"模型与 Harness 共同进化"这个灵魂题做了具象化拆解。
...
—— zhouhao, 2026-05-24T08:53:29+00:00