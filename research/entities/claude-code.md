...
基于孔某人的低维认知对 Claude Code 2.1.156 的逆向([记忆/压缩横评](../comparisons/harness-memory-compaction-implementation.md)):

- 记忆 = 增量编辑路线:主 Session 模型主动写 + 对话结束后 fork session 自动抽取(两路互斥);存 `$HOME` 而非项目目录;MEMORY.md 索引 + 多个 item 文件;Dream 离线整合默认关闭。
- prompt 大量篇幅在规定"不记什么(代码/git 历史/调试套路/临时状态/密钥)"和"召回前先核实(文件/flag 可能已失效)"。
- 压缩 4 种:microcompact / autoCompact / reactiveCompact / precomputedCompact(后两/微压默认关);压缩 prompt 先 `<analysis>` 再 `<summary>`、强制不调工具;支持局部 Summarize up to / from here(类树形 context)。
