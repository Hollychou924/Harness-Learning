...
**A.1 国际话语独有视角**:KV cache / prompt cache / context cache 在中文 63 篇中 DF<5 (8%) ——绝大多数中文 Harness 文章不讨论 cache 优化。**这反而是本报告的差异化价值**:中文社区把"减少 turn"作为成本优化的主路径,硅谷把"复用 KV"作为另一条优化路径,两条路径并行不悖。

**A.2 对 DeepSeek 的隐藏含义**:Cache 是国际市场的成本叙事,中国市场更看"turn 数"和"token 单价"。DeepSeek 在中国推广时,**应当把 KV cache 经济性翻译为"等价于多少次免费调用"这种直觉指标**,而不是直接复用 Anthropic / OpenAI 的 cache hit ratio 话语。否则容易陷入"中国开发者听不懂"的传播困境。
