...
**A.1 关于权限抓手 (§2.2 模式 B)**:中文社区把 Claude Code 的权限边界落到具体 Markdown 文件而非抽象架构——`AGENTS.md` DF=48% (TF=336),`CLAUDE.md` DF=22% (TF=50),分别比 Subagent (TF=183) 高 83% 和高出一倍以上。**对模式 B 的实际描述应当包含"AGENTS.md 文件协议"作为权限抓手**,而不仅是 Subagent 嵌套深度。

**A.2 关于 Stripe 百万行论据 (§2.3 模式 C)**:30% 的中文 Harness 文章 (19/63) 把 OpenAI 那篇 Codex 百万行实验作为唯一定性论据。本报告引用 Stripe 案例时应注明:**这个数字目前在中文社区被循环引用,缺乏独立验证**。一旦真实性被挑战 (例如 Stripe 公开使用细节),整个论证基础有寒冬风险。
