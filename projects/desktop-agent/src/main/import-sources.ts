...
function textFromBlocks(value: unknown): string {
  if (typeof value === 'string') return cleanLeakedWrapperTags(hideSensitiveText(value))
  if (!Array.isArray(value)) return ''
  return value.flatMap((block) => {
    if (!block || typeof block !== 'object') return []
    if (typeof block.text === 'string' && ['input_text', 'output_text', 'text'].includes(block.type)) return [cleanLeakedWrapperTags(hideSensitiveText(block.text))]
    return []
  }).join('\n')
}
...
function cursorUserText(content: string): { text: string; imagePaths: string[] } {
  ...
  if (query !== undefined) return { text: cleanLeakedWrapperTags(hideSensitiveText(query.trim())), imagePaths }
  return {
    text: cleanLeakedWrapperTags(hideSensitiveText(content)),
    imagePaths
  }
}

function cleanCursorAssistantText(content: string): string {
  return hideSensitiveText(content
    .replace(/`?<\/?think>`?/gi, '')
    .replace(/(?:^|\n)\s*```(?:thinking)?\s*\n[\s\S]*?```\s*(?=\n|$)/gi, '\n')
    .replace(/\n{3,}/g, '\n\n')
    .trim())
...
function resource(source: ImportSourceId, kind: ImportedResource['kind'], path: string, projectId?: string): ImportedResource {
  ...
  return {
    ...
    name: basename(dirname(path)) === 'memory' ? basename(path) : basename(path), sourcePath: path,
    content, enabled: false, fingerprint: contentFingerprint(content)
  }
}

function safeJsonResource(source: ImportSourceId, kind: ImportedResource['kind'], path: string): ImportedResource {
  ...
  return {
    id: stableId('resource', `${source}:${kind}:${path}`), source, sourceId: path, kind,
...
function codexMcpResource(path: string): ImportedResource | undefined {
  if (!existsSync(path)) return undefined
  ...
  for (const line of lines) {
    ...
    if (header) {
      include = header === 'mcp_servers' || header.startsWith('mcp_servers.')
      hideSection = header.includes('.env')
    }
    if (!include) continue
    selected.push(hideSection && !header ? '[环境内容已隐藏]' : hideSensitiveText(line))
  }
  const content = selected.join('\n').trim()
  if (!content) return undefined
  return {
    id: stableId('resource', `codex:mcp:${path}`), source: 'codex', sourceId: `${path}#mcp_servers`, kind: 'mcp',
...
function resourcesBelow(source: ImportSourceId, home: string, projects: ImportedProject[]): ImportedResource[] {
  ...
  if (source === 'codex') {
    ...
    if (existsSync(mcpConfig)) result.push(safeJsonResource(source, 'mcp', mcpConfig))
  }
  return result
}
...
export function scanCursor(home = homedir(), installedVersion = cursorVersion(), canReadDatabase = hasSqliteReader()): ImportCandidate {
  ...
  if (existsSync(mcpPath)) candidate.resources.push(safeJsonResource('cursor', 'mcp', mcpPath))
