...
import { countPending, parseClaudeFile, parseCodexFile, parseCursorTranscript, repairStoredSession, scanKnownSources } from './import-sources.js'
...
test('个性化资料覆盖分层说明、外部连接和自动动作且默认关闭', () => {
  ...
  // MCP 需导入后可直接使用，连接配置中的密钥原样保留
  assert.match(resources.filter((item) => item.kind === 'mcp').map((item) => item.content).join('\n'), /private-value/)
  assert.ok(candidates[1].resources.every((item) => item.kind !== 'mcp' || item.content?.includes('private-value')))
})

test('矩阵分类计数区分全局与项目 MCP/Skills', () => {
  const home = temp()
  const project = join(home, 'work/demo')
  jsonLines(join(home, '.cursor/projects/demo/agent-transcripts/aaaa.jsonl'), [
    { role: 'user', message: { content: [{ type: 'text', text: '<user_query>hi</user_query>' }] } }
  ])
  // Cursor 扫描依赖 composer DB；这里只测 Codex/Claude 项目级资源
  jsonLines(join(home, '.codex/sessions/2026/01/a.jsonl'), [
    { type: 'session_meta', payload: { id: 'c1', cwd: project, timestamp: '2026-01-01T00:00:00Z' } },
    { type: 'response_item', payload: { type: 'message', role: 'user', content: [{ type: 'input_text', text: '你好' }] } }
  ])
  mkdirSync(join(project, '.codex/skills/local'), { recursive: true })
  writeFileSync(join(project, '.codex/skills/local/SKILL.md'), '项目技能')
  writeFileSync(join(project, '.codex/config.toml'), '[mcp_servers.local]\ncommand = "echo"\n')
  writeFileSync(join(home, '.codex/AGENTS.md'), '全局')
  mkdirSync(join(home, '.codex/skills/g'), { recursive: true })
  writeFileSync(join(home, '.codex/skills/g/SKILL.md'), '全局技能')

  const { preview, candidates } = scanKnownSources(home, undefined)
  const categories = preview.sources[0].categories
  assert.ok(categories.globalRules >= 1)
  assert.ok(categories.globalSkills >= 1)
  assert.ok(categories.projectSkills >= 1)
  assert.ok(categories.projectMcp >= 1)
  assert.equal(candidates[0].resources.some((item) => item.kind === 'skill' && item.projectId), true)
  assert.equal(candidates[0].resources.some((item) => item.kind === 'mcp' && item.projectId), true)
})
...
test('Codex 用户粘贴 Cursor 内容时剥除残留包裹标签', () => {
  const path = join(temp(), 'codex-paste.jsonl')
  jsonLines(path, [
    { type: 'session_meta', payload: { id: 'codex-paste', cwd: '/work/paste', timestamp: '2026-01-01T00:00:00Z' } },
    { type: 'response_item', payload: { type: 'message', role: 'user', content: [{ type: 'input_text', text: '<user_query>\n把这事做了\n</user_query>\n\n16:37\n**复制**\n[Image]' }] } }
  ])
  const content = parseCodexFile(path).session.messages[0].content
  assert.doesNotMatch(content, /<user_query>|复制|\[Image\]/)
  assert.match(content, /把这事做了/)
})

test('修复历史 Cursor 导入会剥除残留 user_query 标签并补提图片', () => {
  const root = temp()
  ...
  const turns = [{
    id: 'turn-1', status: 'completed', startedAt: 1, items: [
      { type: 'userMessage', id: 'item-0', content: [{ type: 'text', text: messages[0].content }] }
    ]
  }] as any

  const repaired = repairStoredSession('cursor', messages, turns, '/work/cursor')
  assert.equal(repaired.changed, true)
  assert.equal(repaired.messages[0].content, '这部分能不能改')
  assert.doesNotMatch(repaired.messages[0].content, /<user_query>|image_files|\[Image\]/)
  assert.equal(repaired.assets.length, 1)
  assert.equal(repaired.assets[0].size, 9)
  assert.equal((repaired.messages[0].attachments || []).length, 1)
  assert.match((repaired.messages[0].attachments as any[])[0].dataUrl, /^imported-asset:\/\//)
  assert.equal((repaired.turns[0].items[0] as any).content[0].text, '这部分能不能改')
  assert.equal((repaired.turns[0].items[0] as any).content[1].type, 'image')

  // 幂等：再跑一次不会再改动
  const again = repairStoredSession('cursor', repaired.messages, repaired.turns, '/work/cursor')
  assert.equal(again.changed, false)
})

test('修复 Codex 粘贴内容只剥包裹标签、保留全部正文', () => {
  const messages = [{
    role: 'user',
    content: '<user_query>\n第一条\n</user_query>\n\n16:37\n**复制**\n<user_query>\n第二条\n</user_query>'
  }] as any
  const turns = [{
    id: 't', status: 'completed', startedAt: 1, items: [
      { type: 'userMessage', id: 'i', content: [{ type: 'text', text: messages[0].content }] }
    ]
  }] as any
  const repaired = repairStoredSession('codex', messages, turns, '/work')
  assert.equal(repaired.changed, true)
  assert.doesNotMatch(repaired.messages[0].content, /<user_query>|复制/)
  assert.match(repaired.messages[0].content, /第一条.*第二条/s)
  assert.equal(repaired.assets.length, 0)
})

test('修复助手消息里的本地 markdown 图片引用并改写为 imported-asset', () => {
  const root = temp()
  const imagePath = join(root, 'out.png')
  writeFileSync(imagePath, Buffer.from('out-bytes'))
  const messages = [
  ...
  const turns = [{
    id: 't', status: 'completed', startedAt: 1, items: [
      { type: 'userMessage', id: 'i0', content: [{ type: 'text', text: '生成一张图' }] },
      { type: 'agentMessage', id: 'i1', text: messages[1].content, phase: 'final_answer' }
    ]
  }] as any
  const repaired = repairStoredSession('cursor', messages, turns, root)
  assert.equal(repaired.changed, true)
  assert.match(repaired.messages[1].content, /imported-asset:\/\//)
  assert.doesNotMatch(repaired.messages[1].content, new RegExp(imagePath))
  assert.equal(repaired.assets.length, 1)
  assert.equal((repaired.messages[1].attachments || []).length, 1)
  assert.equal((repaired.turns[0].items[1] as any).text, repaired.messages[1].content)
  assert.equal(((repaired.turns[0].items[1] as any).images || []).length, 1)
})

test('Codex 首条注入的 AGENTS.md 指令块会被整条丢弃，对话从真正的提问开始', () => {
  const path = join(temp(), 'codex-agents.jsonl')
  const agentsBlock = '# AGENTS.md instructions\n\n<INSTRUCTIONS>\n# AI助手指令\n全程只用产品语言说话。\n</INSTRUCTIONS>\n<environment_context>\n  <cwd>/work/demo</cwd>\n  <shell>zsh</shell>\n</environment_context>'
  jsonLines(path, [
    { type: 'session_meta', payload: { id: 'codex-agents', cwd: '/work/demo', timestamp: '2026-01-01T00:00:00Z' } },
    { type: 'response_item', payload: { type: 'message', role: 'user', content: [{ type: 'input_text', text: agentsBlock }] } },
    { type: 'response_item', payload: { type: 'message', role: 'user', content: [{ type: 'input_text', text: '把这个项目拉下来' }] } },
    { type: 'response_item', payload: { type: 'message', role: 'assistant', content: [{ type: 'output_text', text: '好的，开始' }] } }
  ])
  const result = parseCodexFile(path)
  // 指令块整条被丢弃，首条用户消息是真正的提问
  assert.equal(result.session.messages.length, 2)
  assert.equal(result.session.messages[0].content, '把这个项目拉下来')
  assert.equal(result.session.messages[1].content, '好的，开始')
  // 标题来自真正的提问，不是 AGENTS.md
  assert.equal(result.session.title, '把这个项目拉下来')
  assert.doesNotMatch(result.session.messages[0].content, /AGENTS\.md|INSTRUCTIONS|环境|cwd/)
})

test('修复历史 Codex 导入会剥掉首条 AGENTS.md 指令块并丢弃空消息及其轮次', () => {
  const agentsBlock = '# AGENTS.md instructions\n\n<INSTRUCTIONS>\n# AI助手指令\n遵守规则。\n</INSTRUCTIONS>\n<environment_context>\n  <cwd>/work/legacy</cwd>\n</environment_context>'
  const messages = [
    { role: 'user', content: agentsBlock },
    { role: 'user', content: '真正的第一个问题' },
    { role: 'assistant', content: '回答' }
  ] as any
  const turns = [
    { id: 'turn-1', status: 'completed', startedAt: 1, items: [
      { type: 'userMessage', id: 'i0', content: [{ type: 'text', text: agentsBlock }] }
    ] },
    { id: 'turn-2', status: 'completed', startedAt: 2, items: [
      { type: 'userMessage', id: 'i1', content: [{ type: 'text', text: '真正的第一个问题' }] },
      { type: 'agentMessage', id: 'i2', text: '回答', phase: 'final_answer' }
    ] }
  ] as any

  const repaired = repairStoredSession('codex', messages, turns, '/work/legacy')
  assert.equal(repaired.changed, true)
  // 空的指令块消息被丢弃
  assert.equal(repaired.messages.length, 2)
  assert.equal(repaired.messages[0].content, '真正的第一个问题')
  assert.equal(repaired.messages[1].content, '回答')
  // 只剩第二个轮次（第一个轮次整轮丢弃）
  assert.equal(repaired.turns.length, 1)
  assert.equal((repaired.turns[0].items[0] as any).content[0].text, '真正的第一个问题')
  // 幂等
  const again = repairStoredSession('codex', repaired.messages, repaired.turns, '/work/legacy')
  assert.equal(again.changed, false)
})

test('AGENTS.md 指令块后若跟了真实提问，只剥指令块、保留提问', () => {
  const path = join(temp(), 'codex-agents-trailing.jsonl')
  const text = '# AGENTS.md instructions\n\n<INSTRUCTIONS>\n规则。\n</INSTRUCTIONS>\n紧接着的提问'
  jsonLines(path, [
    { type: 'session_meta', payload: { id: 'codex-trailing', cwd: '/work/t', timestamp: '2026-01-01T00:00:00Z' } },
    { type: 'response_item', payload: { type: 'message', role: 'user', content: [{ type: 'input_text', text }] } }
  ])
  const result = parseCodexFile(path)
  assert.equal(result.session.messages.length, 1)
  assert.equal(result.session.messages[0].content, '紧接着的提问')
  assert.doesNotMatch(result.session.messages[0].content, /AGENTS\.md|INSTRUCTIONS/)
})

test('Codex 带 recommended_plugins 前导的 AGENTS.md 指令块也会被丢弃', () => {
  const path = join(temp(), 'codex-agents-plugins.jsonl')
  const text = '<recommended_plugins> Here is a list of plugins. </recommended_plugins>\n# AGENTS.md instructions for /work/p\n\n<INSTRUCTIONS>\n规则。\n</INSTRUCTIONS>\n<environment_context>\n  <cwd>/work/p</cwd>\n</environment_context>'
  jsonLines(path, [
    { type: 'session_meta', payload: { id: 'codex-plugins', cwd: '/work/p', timestamp: '2026-01-01T00:00:00Z' } },
    { type: 'response_item', payload: { type: 'message', role: 'user', content: [{ type: 'input_text', text }] } },
