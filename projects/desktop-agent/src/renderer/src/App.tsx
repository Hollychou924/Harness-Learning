  ...
  return (
    <div className="flex h-screen w-screen overflow-hidden relative">
      ...
      <button
        ...
        className={`no-drag fixed left-[80px] top-[7px] z-[60] w-7 h-7 rounded-md flex items-center justify-center transition ${
          sidebarCollapsed
      ...
      {status !== 'idle' && (
        <button
          ...
          className={`no-drag fixed right-2 top-[7px] z-[60] w-7 h-7 rounded-md flex items-center justify-center transition ${
            rightCollapsed
              ? 'bg-black/[0.06] text-[var(--ink)]'
              : 'text-[var(--ink-soft)] hover:bg-black/[0.06] hover:text-[var(--ink)]'
          }`}
